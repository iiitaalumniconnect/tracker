...
#             extra_data = {k: make_json_safe(v) for k, v in row_dict.items()}
...
def process_csv_background(file_path: str, record_id: int):
    ...
    try:
        ...
        for col in df.columns:
            ...
            if norm in ["linkedinurl", "linkedinprofileurl", "linkedin"]:
                column_map["linkedin_url"] = col

            elif norm in ["name", "fullname"]:
                column_map["full_name"] = col
        ...
        for _, row in df.iterrows():
            ...
            linkedin_url = str(row.get(column_map.get("linkedin_url", ""), "")).strip()
            ...
            full_name = str(row.get(column_map.get("full_name", ""), "")).strip()

            company = str(row.get(column_map.get("company", ""), "")).strip()

            designation = str(row.get(column_map.get("designation", ""), "")).strip()

            location = str(row.get(column_map.get("location", ""), "")).strip()

            location = extract_country(location)

            # Remove mapped fields from extra_data
            for mapped_col in column_map.values():
                row_dict.pop(mapped_col, None)
...
async def upload_csv(
    ...
    if len(df) > 500:
        raise HTTPException(
            status_code=400,
            detail=f"File exceeds the maximum limit of 500 rows. You provided {len(df)} rows.",
        )
    ...
    if not linkedin_found:
        raise HTTPException(
            status_code=400,
            detail="LinkedIn URL column not found",
        )
...
def delete_upload_history(file_id: int, db: Session = Depends(get_db)):
    ...
    # Delete related mapping entries first to prevent foreign key constraint violations
...
def download_upload_history(file_id: int, db: Session = Depends(get_db)):
    ...
    # Detect if original uploaded file had a flag column in row 1
    flag_col_idx_orig = None
    try:
        temp_wb = openpyxl.load_workbook(file_path, read_only=True)
        temp_ws = temp_wb.active
        for c in range(1, temp_ws.max_column + 1):
            val = temp_ws.cell(row=1, column=c).value
            val_str = str(val).lower().strip() if val else ""
            fill = temp_ws.cell(row=1, column=c).fill
            color = (
                fill.start_color.rgb
                if fill and fill.fill_type and fill.fill_type != "none"
                else None
            )
            is_flag_name = val_str in ("flag", "changed", "tick", "status")
            is_highlighted_empty = (
                color
                and color in ("FFFFFF00", "00FFFF00", "FFFF00")
                and (not val or val_str.startswith("unnamed"))
            )
            if is_flag_name or is_highlighted_empty:
                flag_col_idx_orig = c
                break
    except:
        pass
    ...
    for _, row in df.iterrows():
        linkedin_url = str(
            row.get(column_map.get("linkedin_url", "linkedin_url"), "")
        ).strip()
        ...
        old_location = (
            extract_country(clean_nan(row.get(column_map.get("location", ""), "")))
            .strip()
            .lower()
        )
        ...
        if alumni:
            ...
            old_loc_norm = normalize_string(old_location).strip().lower()
            latest_loc_norm = (
                normalize_string(extract_country(db_location)).strip().lower()
            )
        ...
        # Write tick to the original flag column if it existed
        if flag_col_idx_orig:
            flag_cell = ws.cell(row=current_row, column=flag_col_idx_orig)
            if has_any_changed:
