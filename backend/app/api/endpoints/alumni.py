...
def read_alumni_list(
    ...
    alumni = alumni_service.get_alumni_list(
        db, skip=skip, limit=limit, query=q, company=company, location=location
    )

    # Filter by upload_id if provided
    if upload_id:
        upload_alumni_ids = (
            db.query(core_models.UploadAlumni.alumni_id)
            .filter(core_models.UploadAlumni.upload_id == upload_id)
            .all()
        )
...
def export_alumni(
    ...
    alumni = alumni_service.get_alumni_list(
        db, skip=0, limit=100000, query=q, company=company, location=location
    )
    ...
    def get_latest_position_info(position_groups):
        ...
        if not isinstance(first_group, dict):
            return "", "", ""
        latest_company = (
            first_group.get("companyName") or first_group.get("company") or ""
        )
        latest_designation = ""
        latest_location = ""
    ...
    if upload_id:
        uploaded_file = (
            db.query(core_models.UploadedFile)
            .filter(core_models.UploadedFile.id == upload_id)
            .first()
        )
        if uploaded_file:
            file_path = os.path.join(
                "uploads", f"{uploaded_file.id}_{uploaded_file.filename}"
            )
                ...
                try:
                    ...
                    for c in range(1, ws.max_column + 1):
                        ...
                        if norm == "linkedinurl" or norm == "linkedinprofileurl":
                            ...
                            "anyhigherqualificationreceivedafterpassingoutfromiiitapleaseprovidedetails",
                            "highereducation",
                            "higheredu",
                            "qualification",
                        ):
                    ...
                    if "flag" not in col_map:
                        for c in range(1, ws.max_column + 1):
                            ...
                            if (
                                color
                                and color in ("FFFFFF00", "00FFFF00", "FFFF00")
                                and (
                                    val is None
                                    or str(val).lower().startswith("unnamed")
                                )
                            ):
                                col_map["flag"] = c
                                cell.value = "Flag"
                    ...
                    if link_col:
                        for r in range(2, ws.max_row + 1):
                            ...
                            old_comp = (
                                clean_val(
                                    ws.cell(row=r, column=col_map["company"]).value
                                )
                            ...
                            old_des = (
                                clean_val(
                                    ws.cell(row=r, column=col_map["designation"]).value
                                )
                            ...
                            old_loc = (
                                extract_country(
                                    clean_val(
                                        ws.cell(row=r, column=col_map["location"]).value
                                    )
                                )
                            ...
                            old_edu = (
                                clean_val(
                                    ws.cell(row=r, column=col_map["higher_qual"]).value
                                )
                                if "higher_qual" in col_map
                                else ""
                            )
                            ...
                            if alumni_rec:
                                ...
                                if db_company:
                                    ws.cell(
                                        row=r, column=col_map["new_company"]
                                    ).value = db_company
                                    if normalize_string(old_comp) != normalize_string(
                                        db_company
                                    ):
                                        ws.cell(
                                            row=r, column=col_map["new_company"]
                                ...
                                if db_designation:
                                    ws.cell(
                                        row=r, column=col_map["new_designation"]
                                    ).value = db_designation
                                    if normalize_string(old_des) != normalize_string(
                                        db_designation
                                    ):
                                        ws.cell(
                                            row=r, column=col_map["new_designation"]
                                ...
                                if db_location:
                                    ws.cell(
                                        row=r, column=col_map["updated_location"]
                                    ).value = db_location
                                    if normalize_string(old_loc) != normalize_string(
                                        extract_country(db_location)
                                    ):
                                        ws.cell(
                                            row=r, column=col_map["updated_location"]
                                        ).fill = yellow_fill
                                ...
                                if (
                                    normalize_string(old_edu)
                                    != normalize_string(latest_higher_edu)
                                    and latest_higher_edu
                                ):
                                    ws.cell(
                                        row=r, column=col_map["new_higher_edu"]
                                    ).value = latest_higher_edu
                                    ws.cell(
                                        row=r, column=col_map["new_higher_edu"]
                                    ).fill = yellow_fill
                                    has_changes = True
                    ...
                    logger.error(
                        f"Template export failed, falling back to scratch export: {ex}"
                    )
    ...
    for a in alumni:
        logs = (
            db.query(core_models.ChangeLog)
            .filter(
                core_models.ChangeLog.alumni_id == a.id,
                core_models.ChangeLog.field_changed.in_(
                    ["full_name", "company", "designation", "location"]
                ),
            )
            .order_by(core_models.ChangeLog.detected_at.asc())
            .all()
        )
    ...
    for a in alumni:
        ...
        if not higher_qual and a.education:
            try:
                education_list = (
                    a.education if isinstance(a.education, list) else [a.education]
                )
                ...
                for edu in education_list:
                    if isinstance(edu, dict):
                        ...
                        degree = (
                            edu.get("degree") or edu.get("fieldOfStudy") or "Degree"
                        )
        ...
        if course and higher_qual:
            degree_part = (
                higher_qual.split(" from ")[0].strip()
                if " from " in higher_qual
                else higher_qual
            )
        ...
        old_name = (
            extra.get("Name") or (f_logs[0].old_value if f_logs else a.full_name) or ""
        )

        c_logs = [l for l in alumni_logs[a.id] if l.field_changed == "company"]
        old_company = (
            extra.get("Company/Organization Name")
            or (c_logs[0].old_value if c_logs else a.company)
            or ""
        )
        ...
        old_designation = (
            ...
            or (d_logs[0].old_value if d_logs else a.designation)
            or ""
        )

        l_logs = [l for l in alumni_logs[a.id] if l.field_changed == "location"]
        old_location = get_full_location(
            extra.get("Work location (City & Country)")
            or (l_logs[0].old_value if l_logs else a.location)
            or ""
        )
        ...
        def format_field(data, field_type):
            ...
            for item in data:
                ...
                def get_date(val):
                    if isinstance(val, dict):
                        start = (
                            val.get("start", {}).get("year")
                            if isinstance(val.get("start"), dict)
                            else val.get("start")
                        )
                        end = (
                            val.get("end", {}).get("year")
                            if isinstance(val.get("end"), dict)
                            else val.get("end")
                        )
                        if start and end:
                            return f"{start} - {end}"
                ...
                if isinstance(positions, list) and positions:
                    for pos in positions:
                        ...
                        p_parts = [f"  -> {pos_title}"]
                        if pos_date:
                            p_parts.append(f"({pos_date})")
                        if pos_loc:
                            p_parts.append(f"- {pos_loc}")
        ...
        cleaned_row = [clean_excel_value(v) for v in row_data]
    ...
    try:
        ...
        response.headers["Content-Disposition"] = (
            "attachment; filename=Alumni_Updates.xlsx"
        )
        return response
    except Exception as e:
        logger.error(f"Error creating Excel export: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Failed to create export file: {str(e)}"
        )
...
def create_alumni(alumni: AlumniCreate, db: Session = Depends(get_db)):
    ...
    if db_alumni:
        raise HTTPException(
            status_code=400, detail="Alumni with this LinkedIn URL already exists"
        )
...
def track_alumni(alumni_id: int, db: Session = Depends(get_db)):
    ...
    return {
        "message": "Tracking pipeline completed successfully",
        "alumni_id": alumni_id,
    }
...
def search_alumni(
    ...
    return alumni_service.get_alumni_list(
        db, query=q, company=company, location=location
    )
...
def delete_alumni(alumni_id: int, db: Session = Depends(get_db)):
    alumni = (
        db.query(core_models.AlumniMaster)
        .filter(core_models.AlumniMaster.id == alumni_id)
        .first()
    )
    if not alumni:
        raise HTTPException(status_code=404, detail="Alumni not found")
    ...
    db.query(core_models.ChangeLog).filter(
        core_models.ChangeLog.alumni_id == alumni_id
    ).delete()
    db.query(core_models.AlumniHistory).filter(
        core_models.AlumniHistory.alumni_id == alumni_id
    ).delete()
...
def get_alumni_changelog(alumni_id: int, db: Session = Depends(get_db)):
    results = (
        db.query(core_models.ChangeLog)
        .filter(core_models.ChangeLog.alumni_id == alumni_id)
        .order_by(core_models.ChangeLog.detected_at.desc())
        .all()
    )
    return results
